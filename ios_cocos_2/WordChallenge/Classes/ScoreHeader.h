...
public:
    ...
    ScoreHeader()
    {
        m_scoreLabel = CCLabelTTF::create("Score: 0", WC_DEFAULT_FONT, 14);
        m_scoreLabel->setColor(ccc3(0,0,0));
        this->addChild(m_scoreLabel);
        this->setPosition(ccp(160,470));
        
        CCNotificationCenter::sharedNotificationCenter()->addObserver(this,callfuncO_selector(ScoreHeader::letterPlaced),WC_EVENT_LETTER_PLACED,NULL);

    }
    
    ~ScoreHeader()
    {
        CCNotificationCenter::sharedNotificationCenter()->removeObserver(this,WC_EVENT_LETTER_PLACED);
